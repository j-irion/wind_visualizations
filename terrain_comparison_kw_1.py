...
# Select a one week period for detailed analysis
start_date = "2020-01-01"
end_date = "2020-01-08"

rolling_hills_week = rolling_hills_data[start_date:end_date]
mountainous_week = mountainous_data[start_date:end_date]
flat_lands_week = flat_lands_data[start_date:end_date]

# Plotting the wind speed for the selected week across the three terrains
plt.figure(figsize=(15, 6))
plt.plot(rolling_hills_week.index, rolling_hills_week['wind speed at 100m (m/s)'], label='Rolling Hills (CA)', color='green')
plt.plot(mountainous_week.index, mountainous_week['wind speed at 100m (m/s)'], label='Mountainous (ID)', color='blue')
plt.plot(flat_lands_week.index, flat_lands_week['wind speed at 100m (m/s)'], label='Flat Lands (NY)', color='red')

plt.title('Wind Speed at 100m over Week 1 of 2020 across Different Terrains')
plt.xlabel('Date')
...
plt.tight_layout()
plt.savefig('plots/terrain_comparison_kw_1.png')
plt.show()
