...
# Resampling data to get weekly averages for the entire year of 2020
rolling_hills_weekly_avg = rolling_hills_data.resample('W').mean()
mountainous_weekly_avg = mountainous_data.resample('W').mean()
flat_lands_weekly_avg = flat_lands_data.resample('W').mean()

rolling_hills_weekly_avg.head(), mountainous_weekly_avg.head(), flat_lands_weekly_avg.head()

# Plotting the weekly average wind speed for the entire year across the three terrains
plt.figure(figsize=(15, 6))

plt.plot(rolling_hills_weekly_avg.index, rolling_hills_weekly_avg['wind speed at 100m (m/s)'], label='Rolling Hills (CA)', color='green')
plt.plot(mountainous_weekly_avg.index, mountainous_weekly_avg['wind speed at 100m (m/s)'], label='Mountainous (ID)', color='blue')
plt.plot(flat_lands_weekly_avg.index, flat_lands_weekly_avg['wind speed at 100m (m/s)'], label='Flat Lands (NY)', color='red')

plt.title('Weekly Average Wind Speed at 100m Over the Year 2020 across Different Terrains')
plt.xlabel('Week')
...
plt.tight_layout()
plt.savefig('plots/terrain_comparison_weekly_averages.png')
plt.show()
